...
2. Go to your repository at [fmacias64/nexus](https://github.com/fmacias64/nexus).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `nexus.localhost` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | nexus.localhost | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/fmacias64/nexus/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fmacias64/nexus/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/fmacias64/nexus-backend`) and Frontend (`ghcr.io/fmacias64/nexus-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of nexus.localhost](https://github.com/fmacias64/nexus/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/nexus.localhost.yml` stack.
