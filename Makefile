...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://nexus.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t fmacias64/nexus-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t fmacias64/nexus-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name nexus-backend-acceptance -d fmacias64/nexus-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name nexus-frontend-acceptance --link nexus-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fmacias64/nexus-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop nexus-frontend-acceptance
	@docker stop nexus-backend-acceptance
